...
Data        : 27/08/2026
Objetivo    : Conferir instâncias do jogo Sudoku !!Utilizando alocação dinâmica
Dificuldade : Média
Uso de IA   : Tive dúvida na alocação de ponteiro de ponteiro de ponteiro, então consultei a IA
-------------------------------------------------------------------------- */
...
int main(){
    ...
    //Número de matrizes
    int n;
    int ***v_matrizes; //Declarando um ponteiro de vetor de matrizes
    ...
    //Alocação dinâmica
    //A matriz irá possuir 9 linhas e 9 colunas
    //Alocando o ponteiro para o vetor de matrizes
    v_matrizes = (int ***) malloc(n * sizeof(int **));
    ...
    for(int i = 0; i < n; i++){ //Alocando o ponteiro que representa as linhas da matriz dentro do ponteiro que traz a quantidade de matrizes
        v_matrizes[i] = (int **) malloc(9 * sizeof(int *));
        if(v_matrizes[i] == NULL){
            printf("A alocacao falhou!");
            return 1;
        }
    ...
    for(int i = 0; i < n; i++){ //Alocando o ponteiro que representa as colunas da matriz
        for(int j = 0; j < 9 ; j++){
            v_matrizes[i][j] = (int *) malloc(9 * sizeof(int));
            if(v_matrizes[i][j] == NULL){
                printf("A alocacao falhou!");
                return 1;
            }
        }
    ...
    //Fazendo na ordem inversa para não perder nenhum endereço

     for(int i = 0; i < n; i++){ //desalocando o ponteiro que representa as colunas da matriz
        for(int j = 0; j < 9 ; j++){
            free(v_matrizes[i][j]);
        }
    }

    for(int i = 0; i < n; i++){//Desalocando o ponteiro de linhas da matriz
        free(v_matrizes[i]);
    }

    free(v_matrizes); //Desalocando o ponteiro de vetor de matrizes
